	...
	*/
	...
	private void buttonOuvrirFicheClick(ActionEvent e) {
	 ...
	 private void remplissagetableViewListeFiches() {
		 List<FicheFrais> lesFichesFrais;
		 if (utilisateur.getFicheFrais().isEmpty()) {
			 lesFichesFrais = FicheFraisDAO.lesFichesFraisOfUtilisateur(utilisateur);
		 } else {
			 lesFichesFrais = utilisateur.getFicheFrais();
		 }
		...
		colNom.setCellValueFactory(new PropertyValueFactory<FicheFrais,String>("nomPrenomUtilisateur"));
